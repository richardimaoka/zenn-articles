...
```log:git log
* 5f433ab (HEAD -> pr-1, origin/pr-1) Merge branch 'main' into pr-1
|\
| * 59518cb (origin/main, main) update b in main
...
rebase の結果、pr-2 ブランチが分岐せず一直線になっています。

```log:git log
* f7edabc (HEAD -> pr-2, origin/pr-2, main) update a in pr-2
* d84f7bf (origin/main) update b in main
...
```sh:コピペして実行
gh pr merge pr-2 --rebase --delete-branch
```
