...
GitHub 上でここまでの git log を確認すると、一直線にコミットが並んでいるように見え、かつ Merge commit2 つが連続しています。
...
以下の折りたたみ部分では、この `no-ff` オプションの動作を確認して、本当に GitHub の merge commit と同じ動作なのかを試します。
